...
void *get_closet_fov_ent_proj(void *localplayer, struct vec3_t *shoot_pos, struct vec3_t view_angle, int weapon_id, struct vec3_t *result_angle, struct vec3_t *result_pos, float *result_time)
{
    ...
    for (int ent_index = 1; ent_index <= get_max_entities(); ent_index++)
    {
        ...
        if (entity == NULL || entity == localplayer || is_ent_dormant(entity) || get_ent_team(entity) == get_ent_team(localplayer))
        {
            continue;
        }
        ...
        if (ent_index <= max_clients && get_ent_lifestate(entity) == 1)
        {
            for (float t = 0.0f; t < config.aimbot.projectile_max_time; t += config.aimbot.projectile_time_step)
            {
                ...
                if (time_rounded > t + config.aimbot.projectile_tolerance_time || time_rounded < t - config.aimbot.projectile_tolerance_time || !is_pos_visible(localplayer, shoot_pos, ent_pos))
                {
                    continue;
                }
...
void projectile_aimbot(void *localplayer, struct user_cmd *user_cmd, int weapon_id)
{
    ...
    if (target_ent == NULL)
    {
        ...
        struct vec3_t direction = {cos(pitch_rad) * cos(yaw_rad), cos(pitch_rad) * sin(yaw_rad), sin(pitch_rad)};
        ...
        if (config.aimbot.projectile_preview.draw_box)
        {
            ...
            struct vec3_t box_color = {config.aimbot.projectile_preview.box_color.r * 255, config.aimbot.projectile_preview.box_color.g * 255, config.aimbot.projectile_preview.box_color.b * 255};
        ...
        {
            struct vec3_t line_color = {config.aimbot.projectile_preview.line_color.r * 255, config.aimbot.projectile_preview.line_color.g * 255, config.aimbot.projectile_preview.line_color.b * 255};
    ...
    if (((config.aimbot.key.use_key && config.aimbot.key.is_pressed) || (!config.aimbot.key.use_key && (user_cmd->buttons & 1) != 0)) && can_attack(localplayer))
    {
        ...
        {
            struct vec3_t timer_color = {config.aimbot.projectile_preview.timer_color.r * 255, config.aimbot.projectile_preview.timer_color.g * 255, config.aimbot.projectile_preview.timer_color.b * 255};
        ...
        {
            struct vec3_t previous_shot_line_color = {config.aimbot.projectile_preview.previous_shot_line_color.r * 255, config.aimbot.projectile_preview.previous_shot_line_color.g * 255, config.aimbot.projectile_preview.previous_shot_line_color.b * 255};
        ...
        if (config.aimbot.projectile_preview.previous_shot_box && result_time != -1.0f)
        {
            ...
            struct vec3_t box_color = {config.aimbot.projectile_preview.previous_shot_box_color.r * 255, config.aimbot.projectile_preview.previous_shot_box_color.g * 255, config.aimbot.projectile_preview.previous_shot_box_color.b * 255};
